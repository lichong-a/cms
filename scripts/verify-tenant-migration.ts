...
async function verify() {
  ...
  try {
    ...
    console.log(`   无租户用户: ${usersWithoutTenant}`)
    console.log(`   默认租户用户: ${usersWithTenant}`)
    if (usersWithoutTenant === 0 && usersWithTenant === totalUsers) {
    ...
    const articlesWithoutTenant = await prisma.article.count({
      where: { tenantId: null }
    })
    ...
    console.log(`   无租户文章: ${articlesWithoutTenant}`)
    console.log(`   默认租户文章: ${articlesWithTenant}`)
    if (articlesWithoutTenant === 0 && articlesWithTenant === totalArticles) {
