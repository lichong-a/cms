...
describe('api-base-url', () => {
  it('normalizes an origin-only API env value', () => {
  ...
  it('normalizes env values that already include /api', () => {
  ...
  it('normalizes env values that already include /api/v1', () => {
  ...
  it('uses the current host with port 3003 for direct LAN/dev access when no public API env is set', () => {
    Object.defineProperty(window, 'location', {
      configurable: true,
      value: new URL('http://192.168.0.10:3001'),
    })
  ...
  it('uses the same browser origin when served behind a reverse proxy on standard ports', () => {
    Object.defineProperty(window, 'location', {
      configurable: true,
      value: new URL('https://cms.example.com'),
    })
